...
def buscar_linkedin_e_dominio(nome_busca):
    ...
    for r in serpapi_search(f'"{nome_busca}" site oficial'):
        link = r.get("link", "")

        if "linkedin.com/company" in link and not linkedin_empresa:
        ...
        if not dominio_oficial:
            ...
            if match:
                ...
                if not any(x in dominio for x in [
                    "linkedin", "facebook", "instagram",
                    "google", "wikipedia", "econodata",
                    "cnpj", "jusbrasil"
                ]):
                    dominio_oficial = dominio
...
def buscar_decisor(nome_busca_original: str):
    ...
    query = f'"{nome_busca_original}" ESG OR Sustentabilidade OR Marketing OR Financeiro site:linkedin.com/in'
    ...
    for r in serpapi_search(query):
        ...
        nome_bruto = titulo.split(" - ")[0].strip()

        partes = nome_bruto.split()
        if len(partes) < 2:
            continue

        return nome_bruto, titulo, link
